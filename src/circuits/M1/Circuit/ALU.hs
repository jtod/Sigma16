...
{- The ALU receives three word inputs: x and y (which are usually the
contents of two registers) and cc (the contents of R15).  It produces
two word outputs: r is a numeric result (typically loaded into the
destination register) and ccnew is the new value to be loaded into the
