...
export function instructionLooper (es) {
    ...
    com.log_emph ("instruction looper starting")    
    let icount = 0
    let countOK = true
    let status = 0
    let pauseReq = false
    ...
    while (continueRunning) {
        com.log_emph("looper: continueRunning")
        ...
        case ab.SCB_relinquish:
            com.log_emph("looper: finished=true")
            finished = true
            break
        default:
        }
...
export function executeInstruction (es) {
    ...
    com.log_emph("executeInstruction")
    clearRegLogging (es)
    clearMemLogging (es)
    clearInstrDecode (es)
...
const op_trap = (es) => {
    console.log (`%c*** op_trap es.thread_host=`
                 + `${es.thread_host}`, 'color: red');
    console.log ("op_trap");
    com.log_emph("op_trap (com.log_emph)");
...
//    console.log (`rx - es.reg[ir_a]=${es.regfile[es.ir_a]}`)
    ...
    com.mode.devlog (`rx ea, `
                     + `idx=${arith.wordToHex4(es.regfile[es.ir_a].get())}`)
...
function rx32_llogicf (es) {
    console.log ("em: rx32_llogicf")
}
function rx32_llogicb (es) {
    console.log ("em: rx32_llogicb")
}
function rx32_lextract (es) {
    console.log ("em: rx32_lextract")
}
