...
// Sigma16 can run either in a browser which provides a
// graphical user interface (GUI), or in a shell which
// provides a text interface.

// Sigma16 provides a command line interface to some of its
// components, including the assembler and linker. In this
// case the interaction uses text commands in a shell, and is
// executed by node.js.  There is no GUI, a browser is not
// required, and this server program is not required.  For
// example: `sigma16 assemble myprog' will translate
// myprog.asm.txt to machine language myprog.obj.txt.

// The GUI runs in a browser, which requires Sigma16 to fetch
// its files from a server via https.  This program is the
// server, and it can run without modification either on an
// Internet host (Heroku) or on a local machine, using
// environment variables to adapt.

// Sigma16 won't run on github pages because github.io
// provides only a static server.  Sigma16 uses concurrent
// processes with shared memory, and (from May 2021) browsers
// require cross origin isolation in order to use shared
// memory.  Since github.io does not serve pages with the
// required headers, it doesn't support Sigma16.  In
// contrast, this program is a web server that enforces cross
// origin isolation, and it works for Sigma16 using a modern
// browser.  It can run on a local computer for offline
// testing, or on an Internet server for production use.
...
// $(SIGMASYSTEM)/src/Sigma16/build/dev/Sigma16
//     development
// $(SIGMASYSTEM)/src/server/Sigma16/build/3.4.2/Sigma16
//     local repository
// /app/build/3.4.2/Sigma16
//     Heroku server

// All executable versions are stored in a build directory,
// with a path of the form .../build/VERSION/Sigma16 where
// VERSION can be a specific version number (e.g. "3.4.2") or
// "release" or "dev".

// The server accesses the files in two steps:

//   1. When the server launches, it sets S16_BUILD_DIR to
//      the build directory.  This depends on S15_RUN_ENV,
//      which indicates whether the program is running
//      locally or on the server, as well as the command line
//      arguments, which can specify the version to run.

//   2. When a client makes a request, the URL cam specify
//      release, dev, or a specific version
...
// The Sigma16 Home Page uses the following URL for the
// "Click to launch Sigma16" link:

//   https://sigma16.herokuapp.com/Sigma16
//     /build/release/Sigma16/Sigma16.html

// The server substitutes the current release number for
// "release", and serves the following file:

//   /app/Sigma16/build/3.4.2/Sigma16/Sigma16.html

// When the top html file requests further URLs, it does so
// relative to build/release and the server translates them
// all to build/3.4.2 (or whatever the current release
// version is).
...
// Query
//   S16_LATEST_RELEASE is reported as response to
//   status/latest query

// Release version
//   S16_RELEASE_VERSION is substituted for 'release' in http
//   path These are typically the same in production but
...
//   heroku login
//   heroku config:set FOOBAR=VALUE
...
// The build directory contains a directory for each version.
// The directory name may be a version number 3.3.1 or dev.
// All versions are launched relative to the build directory.
// This location depends on whether the server is running on
// a local development machine or the Heroku Internet server.

let S16_BUILD_DIR

// If the environment defines an http port (e.g. on the
// Heroku server) that is used; otherwise the default
// S16_LOCAL_PORT is used.  The http port should be between
// 1024 and 49151.  S16_LOCAL_PORT is defined in an
// environment variable, and can be changed to avoid clash
...
// string giving the latest release; the value of the string
// is S16_LATEST_RELEASE.  That value is displayed on the
// Options page, enabling the user to see whether they are
...
// Starting URL with /sigma16 allows for status request for
// future programs
...
// Calculate actual version number.  The http request may ask
// for a specific version (1.2.3) or a symbolically named
// version (release, dev).  A symbolically named version is
// substituted with the corresponding version number which is
...
// Without the res.set statements for Cross-Origin, Chrome
// gives a deprecation warning (April 2021) because shared
// memory requires cross origin isolation.  It is expected
// that Chrome 91 (May 2021) will refuse to create the shared
// array.

// https://developer.chrome.com/blog/enabling-shared-array-buffer/

// SharedArrayBuffer is currently available in Desktop
// Chrome, but from Chrome 91 it will be limited to
// cross-origin isolated pages.  You can make a page
// cross-origin isolated by serving the page with these
// headers:

//   Cross-Origin-Embedder-Policy: require-corp
//   Cross-Origin-Opener-Policy: same-origin

// Once you do this, your page will not be able to load
// cross-origin content unless the resource explicitly allows
// it via a Cross-Origin-Resource-Policy header or CORS
// headers (Access-Control-Allow-* and so forth).
...
// The build directory contains a set of subdirectories, one
// for each build that can be launched.  The subdirectories
// are named with the version number.  The server recognises
// two virtual versions: release and dev.  If either of these
// is launched, the server uses the specific version as
// defined in the environment variables S16_RELEASE_VERSION
// and S16_DEV_VERSION.  A comment at the beginning of this
// file gives the URLs needed to launch any version.
