...
from a source register and inserts those bits into an arbitrary
position in a destination register. The destination field is
overwritten, while other bits in the destination register as well as
all bits in the source register are unchanged.
...
bit.  The size of a field is $\max (0, L-R+1)$. Consequently, if $L<R$
the size is 0. For example:
...
- Size of the field = f-g+1
- In general, $R = L - size + 1$
- Destination field = Rd.f, Rd.f-1, ..., Rd.g
- Check that this works for destination: $R = L - size + 1 = f -
  (f-g+1) + 1 = f - f + g - 1 + 1 = g
- Source field = Re.h, Re.h-1, ..., Re.h-f+g
- Calculate $R$ for source, which is not specified in the instruction.
  $R = L - size + 1 = h - (f-g+1) + 1 = h - f + g - 1 + 1 = h - f + g

The size of a field $L,R$ is $L-R+1$.  The instruction specifies the
destination field as $f,g$.  Only the leftmost index of the source
field is specified explicitly; the rightmost index of the source is
$h+g-f$.  The sizes of the source and destination fields must be the
same, and we can verify this by calculationt the size of the source
field = $h - (h+g-f) + 1 = f-g+1$.
...
| shiftl   |   | D,E,L       | e,10 | Rd := Ra shl b               |
| shiftr   |   | D,E,L       | e,11 | Rd := Ra shr b               |
