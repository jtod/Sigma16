...
whether a specific condition is true.  The bits are indexed from bit 0
(rightmost, or least significant) to bit 15 (the leftmost, or most
significant).  The condition code bits that have specific meanings are
called *flags*.
...
- Bit index: Each flag has an index which gives its bit position in
...
**** The other general registers: R1 through R14
...
Table: Flags for both mask and req registers

| Index | Symbol | Meaning                 |  Hex |
|-------+--------+-------------------------+------|
|     0 | time   | timer interrupt request | 0001 |
...
**** req - interrupt request register
...
This gives 16 values of the opcode:
