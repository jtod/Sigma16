...
# followed by pdflatex Sigma16UserGuideText.
...
# Blank lines in CodeListing environment don't work for html export,
# but they do work for latex export
...
# {{{S16version()}}} is defined by docs/VersionMacro.org, which is
# generated automatically by make SetVersion.  It gives the version
# number and date, and is defined by make setVersion

#+MACRO: S16Version (Version/Date disabled for section export)

# This is a benign redefinition to prevent error/abort on export.
...
# Index items break <p>...</p> for html export, so put them at end of
# paragraph.

# org makeindex appears to work only if there is only one org source file
#+OPTIONS: makeindex
...
- Click *Step*.  The processor executes a single instruction and
  displays the effects on the registers and memory: blue for /using/
  a value, and red for /modifying/ it.  The assembly listing shows
...
- *Assembler* translates a program from assembly language to machine
  language, and shows the assembly listing as well as the object code.

- *Linker* is used to combine a collection of object code modules into
  ...
  do not need the linker, and you can skip it.
...
- *About* gives general information about the software, including the
  version.
...
- The *operands* are *R2,42[R0]*.  R2 is the /destination/, where the
  value will be placed.  The value to be put into the destination
  is 42.  For now, ignore the *[R0]*; its purpose will be described
  later.
...
- Decimal numbers are written as strings of digits, optionally
  ...
  23 and 0023 both mean the same (twenty three).

- Hexadecimal numbers are written as four hex digits, and in assembly
  language programs they are indicated by putting $ before the number.
  Thus $00a5 and 0165 both represent the integer 165.
...
- Go to the *Examples* page.  Click *Examples*, then *Core*, then
  *Simple*.  This brings up a list of small and simple example
  programs.  Click *ConstArith.asm.txt*.  You should see the listing
  of the program.  The file name has an extension *.asm.txt* that
  identifies the file as an assembly language source program.
...
the *memory*.  The memory contains a sequence of *memory locations*,
each of which can hold a word (16 bits).  Each location is identifed
by an *address*, which is a natural number $0, 1, 2, 3, \ldots,
65535$.  The notation *M[a]* denotes the value stored in the memory
...
- There are only a few registers available, so we can't keep all the
...
/address/ of the location that contains the variable.  This allows you
...
- *RRR* instructions have an operand field containing three registers
  separated by commas.  Example: *add R8,R13,R6*.
  
- *RX* instructions have an operand field that specifies a register
  and an address.  The address is a name or constant, followed by a
  register. Examples: *load R12,array[R6]* and *lea R5,23[R0]*.
...
- *Starter program.* Editor tab. Click Hello World.  This will load a
  small fixed example program into the editor buffer.
- *Collection of examples.* Examples tab.  The Sigma15 app provides a
...
- *Type it in.* Editor tab. Type in a program.  If there was already
  some text there, click New.
- *Read from a file.* Editor tab. Click Open File.  This brings up a
  file chooser dialog where you can traverse the file system on your
  computer and select a file.
- *Copy from file and paste.* If your program is visible in another
...
*Save your work.* After typing in a program, or editing it, you should
...
contains just one form of conditional, the *conditional goto*.  This
uses a Boolean expression /bexp/ to decide whether to jump to
/someLabel/, or not to jump:

#+BEGIN_CodeListing
if bexp then goto someLabel
#+END_CodeListing

The commonest case is where /bexp/ is a comparision between two
integers:
...
1. First, a *comparison* instruction is used to produce a Boolean
   result, which is placed in the /condition code/ (which is in R15).

2. Second, a *conditional jump* instruction will either jump or not
...
status, R15 also has a name: it's called the /condition code/.
...
*machine language*.  The hardware runs the machine language --- it's
just looking at the numbers.  The text notation with names -- e.g. add
R13,R6,R9 -- is called *assembly language*.  Assembly language is for
...
- op -- Operation code (e.g. add, sub, etc.)  This is a number that
  specifies which instruction it is.
...
1. mul is an RRR instruction, so we need to find the values of the
   four fields: op, d, a, b.
2. The table above says that the opcode for mul is $2 (that's 0010 as
   a bit string).
...
6. Putting the fields together ("assembling them into an
   instruction"), we get the machine language instruction: $2e6a.
...
- The first operand is a register, and it's called the "register
  operand" (e.g. R5)
- The second operand is a constant or label followed by a register
  (e.g. 19[R0] or x[R0]).  This is called the "address operand", and
  it contains two parts:
  - The first part is either a constant like 19, or a label like x.
    ...
    like x, the value is the address in memory of x.
  - The seocnd part is a register in square brackets, e.g. [R0].  This
...
- b contains a code indicating which RX instruction this is (1
  ...
  say which RX instruction it is.
...
- First word:
  - op = $f (indicates that this is an RX instruction, but not which
    one; that needs the b field)
  - d = $7 (the register operand)
  - a = $c (part of the address operand: the register [R12])
  - b = $1 (this is the secondary opcode, which indicates that this RX
    instruction is load)
...
1. Edit the last statement of the program to give y the specified
   value.
2. Execute the program manually, with paper and pencil but without the
   computer.  Give the final values of the registers.  Think about
   what is going on as the program runs.
3. Assemble the program and boot it in the processor.  Run the program
   ...
   at the registers to see if you were right.
...
5. When y=-5424 the program goes into an infinite loop. R1=ead0 (the
   ...
   What started out as the *add* instruction at x has been transformed
   into *jump 7[R0]*, comprising the word at *x* (f003) and the
...
For example, andw R1,R2 calculates the logical /and/ of R1 and R2, and
...
Sigma16 provides an instruction /extract/ which solves this problem
...
- 8 is the index of the rightmost bit in the destination where the
  field will be placed at ascending bit indices
- 4 is the index of the rightmost bit of the field in the source
  register
- 7 is the index of the leftmost bit of the field in the source
  register
...
file.  Click Save and the text in the editor buffer will be written to
a file on your computer.  Depending on how the system is configured,
there may be a dialogue box asking you for a file name, or a generic
default file name may be used (for example, "S16DownloadFile (2).txt"
...
# strict limitations on the ability of applications to access your
# file system.  That's a good feature of browsers -- you don't want a
# malicious web page to start reading, deleting, or corrupting your
# files -- but it does make it inconvenient to save your edited
# assembly programs.
...
The notations $x_i$ and =x.i= both mean the bit with index $i$ in the
...
- A 4-digit hexadecimal constant, where the digits are 0-9 a-f.  The
  constant is preceded by $, for example $3e8c.
...
The *register file* is a set of 16 general registers, each of which
...
automatically loaded into R15, which is also called the /condition
code/.  The description of each instruction states whether R15 is
...
called /flags/.  Each flag gives the status of a relation or event.
...
Register R15 is also called the /condition code/.  Some instructions
...
- Index is the bit position of the flag in the condition code.  Bits
  are numbered from right to left, starting with 0.
- Symbol: the graphical user interface shows the symbol for each bit
  ...
  for both integer and two's complement representation.
- Meaning: Description using English or mathematical notation
- Hex: The value of the condition code word assuming the given flag is
...
instruction to be executed (/not/ the address of the instruction
...
- An error that occurs while executing an instruction.  For example,
  ...
  called an /exception/, and sometimes it is called an interrupt.
- An external event that has nothing to do with the running program,
...
1. If interrupts are not enabled, the check terminates and the next
   instruction is executed normally.
2. If interrupts are enabled, and there is an interrupt request, and
   ...
   index of the type of interrupt that was selected.
3. The processor clears the request bit for the interrupt in the
   ...
   status register to indicate system state and interrupts disabled.
4. If the timer is running, it is stopped.
5. Now the interrupt has completed, and the next instruction is
   executed normally.  Since the pc has been modified, the machine has
   jumped to a different location.
...
as a collection of several *fields*.  A field is a portion of a word
...
instructions, most of which are represented with the *EXP format*. The
name EXP stands simultaneously for *expansion* (because it provides
for many additional instructions) and *experimental* (because it
...
- If the opcode is between 0 and 12 (hex 0 to hex c), the instruction
  is RRR format, and there are 14 RRR instructions.
- An opcode value of 13 is reserved for future expansion of the
  architecture.
- If the op field is 14 (hex e) the instruction is EXP format and has
  a secondary opcode in the a and b fields.
- If the op field contains 15 (hex f) the instruction is RX format
  with a secondary opcode in the b field.
...
- op (bits with index 15 to 12) is the operation code, usually called
  /opcode/.  This determines the operation to be performed.  If the
  opcode is between 0 and 12 it specifies an RRR instruction.  An
  opcode greater than 12 indicates an /expanding opcode/: the
  ...
  This is explained in the sections on RX and EXP formats.
...
primary opcode (op) is =$e=, and the secondary opcode (ab) is =$14=.
...
The *load effective address* instruction *lea Rd,disp[Ra]* calculates
...
This =restore= instruction has the same operands as =save=, and it
...
There are two conditional branch instructions that perform a branch
based on the value of a bit in a register.  For example, brc0 R7,3,xyz
means "if bit 3 of R7 is 0 then go to xyz".
...
brc1 is similar to brc0, but it branches if the specified bit is 1.
...
brnz is similar, but it branches if the register is not zero (i.e. any
bit in Rd is 1).  For example, brnz R3,dest means "if R3 !=0 then goto
dest".
...
- =logicf= performs bitwise logic on corresponding fields within two
  operand registers; the result overwrites the field in the first
  operand.
- =logicb= performs logic on two operand bits that may be at arbitrary
...
The commonest Boolean operators are =inv=, =and2=, =or2=, and =xor2=.
The =inv= function (also called invert, not, logical negation) has one
...
The =and2=, =or2=, =xor2= functions have two inputs, so their truth
tables have four lines.  There are also functions with more inputs,
such as =and3= with 3 inputs, =and4= with 4 inputs, and so on.
Sigma16 doesn't have instructions for those, but you can replace them
with multiple uses of =and2=, =or2=, etc.
...
1, for each result.  For example, the result column for the =and2=
function is (reading from top to bottom) 0001, while =xor2= is 0110.
...
If $(a,b,c,d) = (0,0,0,1)$ this is the truth table for =and2=.  if
$(a,b,c,d) = (0,1,1,0)$ it is the truth table for =xor2=.  Since there
...
The =inv= function can be treated as a 2-input function where the
...
Several special cases of =logicf= appear frequently.  There are
...
For example, suppose you want to calculate the logical /and/ of two
...
$f=0$ and $g=15$.  The instruction =logicf Rd,Re,0,15,h= applies h
...
generates the =logicf= instruction corresponding to any of =invw=,
...
The bit logic instruction =logicb= specifies two operand bits, which
...
- =copyb Rd,Re,f,g= copies a bit from register Re at index g into
  register Rd at index f.
- =copybi Rd,Re,f,g= is similar to copyb, except that it inverts the
  bit as it is written into the destination.
...
The instruction shiftl =Rd,Re,h= shifts the value in the operand
...
The instruction =shiftr Rd,Re,h= shifts the value in the operand
...
*Machine language.* The machine language format of extract is EXP,
...
+--------------+----+----------+------------------------+
| add Rd,Ra,Rb |  0 | $0 d a b | R[d] := R[a] + R[g]    |
|              |    |          | R15 ~=  gGElvVC        |
+--------------+----+----------+------------------------+
...
+--------------+----+----------+------------------------+
| div d,a,b    |  3 | $3 d a b | R[d] := R[a] / R[b]    |
|              |    |          | R[15] := R[a] % R[b]   |
+--------------+----+----------+------------------------+
...
+--------------+----+----------+------------------------+
| muln d,a,b   |  6 | $6 d a b | Rd := Ra :*: Rb        |
+--------------+----+----------+------------------------+
|              |    |          | R15 :=                 |
+--------------+----+----------+------------------------+
| divn d,a,b   |  7 | $7 d a b | R15++Rd := R15++Ra     |
+--------------+----+----------+------------------------+
|              |    |          | :/: Rb                 |
+--------------+----+----------+------------------------+
|              |  8 | $8 - - - | nop                    |
...
+---------------------+-----+-----------+------------------------+----+
| Assembly            | Op  | Code      | Effect                 | CC |
+---------------------+-----+-----------+------------------------+----+
| lea Rd,disp[Ra]     | f,0 | $f d a $0 | R[d] := disp+R[a]      |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
| load Rd,disp[Ra]    | f,1 | $f d a $1 | R[d] := M[disp+R[a]]   |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
| store Rd,disp[Ra]   | f,2 | $f d a $2 | M[disp+R[a]] := R[d]   |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
| jump disp[Ra]       | f,3 | $f d a $3 | pc := disp+R[a]        |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
| jumpc0 d,disp[Ra]   | f,4 | $f d a $4 | if R[15].d=0           |    |
|                     |     | disp      | then pc := disp+R[a]   |    |
+---------------------+-----+-----------+------------------------+----+
| jumpc1 d,disp[Ra]   | f,5 | $f d a $5 | if R[15].d=1           |    |
|                     |     | disp      | then pc := disp+R[a]   |    |
+---------------------+-----+-----------+------------------------+----+
| jal Rd,disp[Ra]     | f,6 | $f d a $6 | R[d]:=pc,              |    |
|                     |     | disp      | pc:=disp+R[a]          |    |
+---------------------+-----+-----------+------------------------+----+
| jumpz Rd,disp[Ra]   | f,7 | $f d a $7 | if R[15].d=0           |    |
|                     |     | disp      | then pc := disp+R[a]   |    |
+---------------------+-----+-----------+------------------------+----+
| jumpnz Rd,disp[Ra]  | f,8 | $f d a $8 | if R[15].d=1           |    |
|                     |     | disp      | then pc := disp+R[a]   |    |
+---------------------+-----+-----------+------------------------+----+
| testset Rd,disp[Ra] | f,9 | $f d a $9 | R[d]:=M[disp+R[a]],    |    |
|                     |     | disp      | M[disp+R[a]]:=0001     |    |
+---------------------+-----+-----------+------------------------+----+
|                     | f,a | $f d a $a | nop, reserved          |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
|                     | ... |           | ...                    |    |
+---------------------+-----+-----------+------------------------+----+
|                     | f,f | $f d a $f | nop, reserved          |    |
|                     |     | disp      |                        |    |
+---------------------+-----+-----------+------------------------+----+
...
+--------------------+----+------+-----+---------------------------+
|logicf Rd,Re,f,g,h  |e,00|EXP   |Ed00 |Rd.f:g := h#(Rd.f:g,Re.f:g)|
|                    |    |RRkkk |efgh |                           |
+--------------------+----+------+-----+---------------------------+
|invw Rd             |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |R     |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|andw Rd,Re          |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|orw Rd,Re           |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|xorw Rd,Re          |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|invf Rd,f,g         |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |R     |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|andf Rd,Re,f,g      |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|orf Rd,Re,f,g       |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|xorf Rd,Re,f,g      |    |pseudo|Ed01 |logicf Rd,Re,0,15,1        |
|                    |    |RR    |e0F1 |                           |
+--------------------+----+------+-----+---------------------------+
|logicb Rd,Re,f,g,h  |e,01|EXP   |Ed01 |Rd.f := h(Rd.f,Re.g)       |
|                    |    |RRkkk |efgh |                           |
+--------------------+----+------+-----+---------------------------+
|invb Rd,Re,f,g      |    |pseudo|Ed01 |logicb Rd,Re,f,g,1         |
|                    |    |RRkk  |efg1 |                           |
+--------------------+----+------+-----+---------------------------+
|andb Rd,Re,f,g      |    |pseudo|Ed01 |logicb Rd,Re,f,g,1         |
|                    |    |RRkk  |efg1 |                           |
+--------------------+----+------+-----+---------------------------+
|orb Rd,Re,f,g       |    |pseudo|Ed01 |logicb Rd,Re,f,g,1         |
|                    |    |RRkk  |efg1 |                           |
+--------------------+----+------+-----+---------------------------+
|xorb Rd,Re,f,g      |    |pseudo|Ed01 |logicb Rd,Re,f,g,1         |
|                    |    |RRkk  |efg1 |                           |
+--------------------+----+------+-----+---------------------------+
|shiftl Rd,Re,h      |e,03|EXP   |     |Rd := Re shl h             |
|                    |    |RRk   |     |                           |
+--------------------+----+------+-----+---------------------------+
|shiftr Rd,Re,h      |e,04|EXP   |     |Rd := Re shr h             |
|                    |    |RRk   |     |                           |
+--------------------+----+------+-----+---------------------------+
|extract Rd,Re,f,g,h |e,05|EXP   |     |Rd.f~g :=                  |
|                    |    |RRkkk |     |                           |
+--------------------+----+------+-----+---------------------------+
|push Rd,Re,Rf       |e,07|EXP   |     |M[ea] := Rd, Rb++          |
|                    |    |RRR   |     |                           |
+--------------------+----+------+-----+---------------------------+
|pop Rd,Re,Rf        |e,08|EXP   |     |Rb--, Rd := M[ea]          |
|                    |    |RRR   |     |                           |
+--------------------+----+------+-----+---------------------------+
|top Rd,Re,Rf        |e,09|EXP   |     |Rb--, Rd := M[ea]          |
|                    |    |RRR   |     |                           |
+--------------------+----+------+-----+---------------------------+
|save Rd,Re,gh[Rf]   |e,0a|EXP   |     |M[Rf+gh] := Rd             |
|                    |    |RRkR  |     |M[Rf+gh+1] := Rd+1         |
|                    |    |      |     | ...                       |
|                    |    |      |     |M[Rf+gh+e-d] := Re         |
+--------------------+----+------+-----+---------------------------+
|restore Rd,Re,gh[Rf]|e,0b|EXP   |     |Rd := M[Rf+gh]             |
|                    |    |RRkR  |     |Rd+1 := M[Rf+gh+1]         |
|                    |    |      |     | ...                       |
|                    |    |      |     |Re := M[Rf+gh+e-d]         |
+--------------------+----+------+-----+---------------------------+
|brc0 Rd,e,fgh       |e,0c|EXP   |Ed0C |if R[d].e=0                |
|                    |    |Rkk   |efgh | then pc:=pc+fgh           |
+--------------------+----+------+-----+---------------------------+
|brc1 Rd,e,fgh       |e,0d|EXP   |Ed0D |if R[d].e=1                |
|                    |e,0d|Rkk   |efgh | then pc:=pc+fgh           |
+--------------------+----+------+-----+---------------------------+
|brz Rd,efgh         |e,0e|EXP   |Ed0E |if R[d]=0000               |
|                    |    |Rk    |efgh | then pc:=pc+efgh          |
+--------------------+----+------+-----+---------------------------+
|brnz Rd,efgh        |e,0f|EXP   |Ed0F |if R[d]!=0000              |
|                    |    |Rk    |efgh | then pc:=pc+efgh          |
+--------------------+----+------+-----+---------------------------+
|dispatch Rd,efgh    |e,10|EXP   |Ed10 |pc := M[pc+min(Rd,efgh)]]  |
|                    |    |Rk    |efgh |                           |
+--------------------+----+------+-----+---------------------------+
|getctl Rd,c         |e,11|EXP   |     |Rd := Sc privileged        |
|                    |    |RC    |     |                           |
+--------------------+----+------+-----+---------------------------+
|putctl Rd,c         |e,12|EXP   |     |Sc := Rd privileged        |
|                    |    |RC    |     |                           |
+--------------------+----+------+-----+---------------------------+
|resume              |e,13|EXP   |     |pc := ipc, (privileged)    |
|                    |    |      |     | status := istatus         |
|                    |    |      |     |                           |
+--------------------+----+------+-----+---------------------------+
|timon Rd,efgh       |e,14|EXP   |     |timer := efgh,             |
|                    |    |R     |     |start timer                |
+--------------------+----+------+-----+---------------------------+
|timoff Rd,efgh      |e,15|EXP   |     |stop timer                 |
|                    |    |R     |     |                           |
+--------------------+----+------+-----+---------------------------+
|                    |e,16|      |     | reserved: nop             |
+--------------------+----+------+-----+---------------------------+
...
- Some assembly language statements are *pseudoinstructions*.  These
  ...
  is a pseudoinstruction which generates a *logicf* instruction
...
Add the following to your .bashrc file, but replace /Users/ yourlogin/
Documents/ path/ to/ with your own file location.  In a bash shell
running on cygwin, try /Users/ yourlogin.
...
- /jumpf is renamed to jumpz, and jumpt is renamed to jumpnz./ The new
  ...
  accurately what the instructions do.  The decision about whether to
  jump depends on whether the entire register contains 0; it isn't a
  decision based on checking just a single bit.)
...
- The word logic instructions =inv=, =and=, =or=, =xor= are now
  pseudoinstructions that generate the =logicf= instruction.  The
...
# The latex printindex gives page numbers for the indexed keywords.
# The index produced by org is well suited for html but not latex; it
# doesn't give page numbers.  Don't want this to be included in the
# latex version.  Maybe don't use include, but instead conditionally
# insert a link?
...
This example starts with ordinary spaces, and all but one are ignored
by the exporter, so this instruction won't be properly indented:

#+BEGIN_CodeListing
     lea  R3,42     ; R3 := 42
#+END_CodeListing

This example has a special space character at the beginning.  It isn't
obtrusive in the text editor, bnt it ensures that the html output
won't ignore the leading spaces:
...
** Status of Examples/Standard/Testing
...
*** Branch
- needs redesign, the test program is no longer consistent with the
  program
*** Ctl.asm.txt
- check execution
*** Dispatch
...
logicr instruction is deprecated

| logicr Rd,e,f,g,h   | e,01 |          | Rd.e := Rd.f #h Rd.g |    |
+---------------------+------+----------+----------------------+----+
...
*** Deprecated: extracti
- Probably won't be used very often; can always do invf after an
  extract to get the same effect
  
+--------------------+----+------+-----+---------------------------+
|extracti Rd,Re,f,g,h|e,06|EXP   |     |Rd := ex Rd di             |
|                    |    |RRkkk |     |                           |
...
;    extracti  Rd,Re,f,g,h
;        EXP. op=#e, ab=$06, g is ignored
;      format: EXP/RRkkk
     extract  R3,R9,5,7,8     ; e306 9578

*** DEPRECATED: Bit logic within a register: logicr
...
- /Partial compatibility./ Most components of the Sigma16 app run on
  *Safari* and *Firefox*, including the assembler, linker, and
  emulator.  However, those browsers do not support local file access,
  so the buttons that read and write files don't work.  However, you
  can copy and paste assembly language programs between an external
  editor and the Sigma16 editor.
- /Full compatibility./ This version of Sigma16 runs on *Chrome*,
  *Edge*, *Opera*, and *DuckDuckGo*.  These browsers are based on the
  Chromium engine.
