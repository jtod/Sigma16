...
Sigma32 is an extension of Sigma16 to 32 bit words.  It
...
than that, as it depends on the browser and the primary
...
- To switch the system to 32 bit mode, open the browser
  ...
  console, enter *setArch32()*.
- To go back to the default 16 bit mode, enter *setArch16()*.
- In the future, the GUI will provide buttons to switch
  modes.

The 32 bit instructions are represened in EXP format.
