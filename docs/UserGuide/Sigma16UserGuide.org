...
The Sigma16 system contains a Core subset, and a full 16-bit
Standard architecture.  The Core is sufficient for beginners and
for a short course on computer systems.  The full Standard
architecture is useful for large scale programming.  The tutorials
are separated into Core and Standard parts.  The system hardware doesn't treat
the Core differently; the hardware doesn't know or care whether an
instruction is part of the Core.
...
|-----------------------+--------+---------------------------|
| [[add][add]] Rd,Ra,Rb | 0dab   | R[d] := R[a]+R[g]         |
| 0 / aRRR              |        | R15 := (cc flags)         |
|-----------------------+--------+---------------------------|
| [[sub][sub]] Rd,Ra,Rb | 1dab   | R[d] := R[a]-R[b]         |
| 1 / aRRR              |        | R15 := (cc flags)         |
|-----------------------+--------+---------------------------|
| [[mul][mul]] d,a,b    | 2dab   | R[d] := R[a]*R[b]         |
| 2 / aRRR              |        | R15 := (cc flags)         |
|-----------------------+--------+---------------------------|
| [[div][div]] d,a,b    | 3dab   | R[d] := Ra / Rb,          |
| 3 / aRRR              |        | R[15] := Ra % Rb          |
|-----------------------+--------+---------------------------|
| [[cmp][cmp]] d,a      | 4da_   | R[15] := R[a] cp R[b]     |
| 4 / aRR               |        | R15 := (cc flags)         |
|-----------------------+--------+---------------------------|
| [[addc][addc]] d,a,b  | 5dab   | Rd := Ra + Rb + R15.C)    |
| 5 / aRRR              |        | R15 := (cc flags)         |
|-----------------------+--------+---------------------------|
| [[muln][muln]] d,a,b  | 6dab   | Rd := Ra :*: Rb, R15 :=   |
| 6 / aRRR              |        |                           |
|-----------------------+--------+---------------------------|
| [[divn][divn]] d,a,b  | 7dab   | R15++Rd := R15++Ra :/: Rb |
| 7 / aRRR              |        | nop                       |
|-----------------------+--------+---------------------------|
|                       | 8___   | nop -- available          |
...
|-----------------------+--------+---------------------------|
| [[trap][trap]] d,a,b  | Cdab   | interrupt                 |
| C / aRRR              |        |                           |
|-----------------------+--------+---------------------------|
...
| *Assembly*                       | *Code* | *Effect*             |
|----------------------------------+--------+----------------------|
| [[lea][lea]] Rd,disp[Ra]         | Fda0   | Rd := disp+Ra        |
| F,0 / aRX                        | disp   |                      |
|----------------------------------+--------+----------------------|
| [[load][load]] Rd,disp[Ra]       | Fda1   | Rd := M[disp+Ra]     |
| F,1 / aRX                        |        |                      |
|----------------------------------+--------+----------------------|
| [[store][store]] Rd,disp[Ra]     | Fda2   | M[disp+Ra] := Rd     |
| F,2 / aRX                        | disp   |                      |
|----------------------------------+--------+----------------------|
| [[jump][jump]] disp[Ra]          | Fda3   | pc := disp+Ra        |
| F,3 /aX                          | disp   |                      |
|----------------------------------+--------+----------------------|
| [[jumpc0][jumpc0]] d,disp[Ra]    | Fda4   | *if* R15.d=0         |
| F,4 / akX                        | disp   | *then* pc := disp+Ra |
|----------------------------------+--------+----------------------|
| [[jumpc1][jumpc1]] d,disp[Ra]    | Fda5   | if R15.d=1           |
| F,5 / akX                        | disp   | then pc := disp+Ra   |
|----------------------------------+--------+----------------------|
| [[jal][jal]] Rd,disp[Ra]         | Fda6   | Rd:=pc,              |
| F,6 / aRX                        | disp   | pc:=disp+Ra          |
|----------------------------------+--------+----------------------|
| [[jumpz][jumpz]] Rd,disp[Ra]     | Fda7   | if R15.d=0           |
| F,7 / aRX                        | disp   | then pc := disp+Ra   |
|----------------------------------+--------+----------------------|
| [[jumpnz][jumpnz]] Rd,disp[Ra]   | Fda8   | if R15.d=1           |
| F,8 / aRX                        | disp   | then pc := disp+Ra   |
|----------------------------------+--------+----------------------|
| [[testset][testset]] Rd,disp[Ra] | Fda9   | Rd:=M[disp+Ra],      |
...
<<copyb>>
<<copybi>>
*** Pseudoinstructions: bit manipulation
...
See the [[mul][mul]] instruction.  This is an internal reference
to mul, and the target is defined by ~<<mul>>~, which is placed
immediately before the place you want the link to go to.

Also see [[mul]].  And there is a section on [[*div][the divide
instruction]].
...
2019-2025 John T. O'Donnell.  Contact email://
