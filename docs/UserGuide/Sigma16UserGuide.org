...
The processor contains several registers to implement interrupts.
...
that are represented with the *EXP format*. The name EXP stands
...
- op (bits 0-3)   4-bit opcode, determines instruction format
- d  (bits 4-7)   4-bit destination
- a  (bits 8-11)  4-bit operand a
- b  (bits 12-15) 4-bit operand b, or expanded opcode for RX
...
opcode (op for short).  This gives 16 values of the opcode:

- If the opcode is between 0 and 13 (hex 0 to hex d), the instruction
...
| assembly language  | save Rd,Re,gh[Rf]            |
| instruction format | EXP                          |
| machine language   | [$e d $0c] [e f gh]          |
...
jumpc0 is used to jump to a destination if a specified bit in the
condition code is 0.
...
| instruction format | RX                         |
| opcode             | f,5                        |
| machine language   | {$f d a $5} {disp}         |
| privileged         | no                         |
| effect             | R15.d=1 => pc := disp+R[a] |
|--------------------+----------------------------|
...
** Table of RRR instructions

+---------+-------------+------+-------------+----------------------+
| Name    |  Operands   | Op   | Code        | Effect               |
+---------+-------------+------+-------------+----------------------+
| add     |   d,a,b     |   0  | $0 d a b    | R[d] := R[a] + R[g]  |
+---------+-------------+------+-------------+----------------------+
| sub     |   d,a,b     |   1  | $1 d a b    | R[d] := R[a] - R[b]  |
+---------+-------------+------+-------------+----------------------+
...
|         |             |      |             | R[15] := R[a] % R[b] |                     |
+---------+-------------+------+-------------+----------------------+
| cmp     |   d,a       |   4  | $4 d a b    | R[15] := R[a] cp R[b]|
+---------+-------------+------+-------------+----------------------+
...
|         |             |      |             |  :/: Rb              |
+---------+-------------+------+-------------+----------------------+
|         |   d,a,b     |   8  | reserved    | nop                  |
+---------+-------------+------+-------------+----------------------+
|         |   d,a,b     |   9  | reserved    | nop                  |
+---------+-------------+------+-------------+----------------------+
|         |   d,a,b     |   a  | reserved    | nop                  |
+---------+-------------+------+-------------+----------------------+
|         |   d,a,b     |   b  | reserved    | nop                  |
+---------+-------------+------+-------------+----------------------+
| trap    |   d,a,b     |   c  | $c d a b    | interrupt            |
+---------+-------------+------+-------------+----------------------+
|         |             |   d  | reserved    | nop                  |
+---------+-------------+------+-------------+----------------------+
|         |             |   e  | secondary op| EXP format           |
+---------+-------------+------+-------------+----------------------+
|         |             |   f  | escape      | RX format            |
+---------+-------------+------+-------------+----------------------+
|         |             |      |             |                      |
