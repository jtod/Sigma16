...
teaching in computer systems.  This application provides a complete
environment for experimenting with the architecture.  It includes an
editor, assembler, linker, emulator, and an integrated development
environment (IDE) with a graphical user interface.
#+INDEX:architecture
#+INDEX: assembler
...
#+BEGIN_majornote
Sigma16 Version 3.8 runs on up-to-date versions of the following
browsers: Chrome, Edge, Opera, DuckDuckGo.  It does not work on Safari
or Firefox.
#+END_majornote
...
functions, and procedures.  They also support systems programming,
including emulators, interpreters, and operating system kernels.
...
Sigma16 provides pseudoinstructions for the commonest logic operations
on words and bits.  This section introduces a few of the logic
instructions, which are adequate for many applications.  There are
additional instructions that are more general and flexible.  See the
section on logic instructions below.

The word logic operations are invw, andw, orw, xorw.  These are
"bitwise" operations: bit $i$ of the result depends on bit $i$ of each
operand, and does not depend on bits at any other index.
...
bits, which may be at any index in any register.  For example, andb
R1,R2,3,9 calculates R1.3 and R2.9 and stores the result in R1.3.
...
The pseudoinstructions shown above are common special cases.  The
architecture provides two general instructions, logicf and logicb,
which can operate on fields within a word, or on individual bits.
Furthermore, those two general instructions implement all possible
logic functions on two operands, not just inv, and, or, xor.  The
instructions described in this section are actually
pseudoinstructions: the assembler generates an equivalent logicf or
logicb instruction.  See the section below on Logic for a complete
description.
...
There are instructions that operate on individual bits in a word, or
on a field of bits within a word, without changing any of the other
bits in the word.
...
- A 4-digit hexadecimal constant, where the digits are 0-9 a-f.
  ...
  hex digits (e.g 3b2f).
  - In assembly language programs, hex constants are written with a
    preceding $ sign (e.g. $3b2f).  This is necessary to avoid
    ambiguity: 1234 is a decimal number and $1234 is a hexadecimal
    number.
  - In contexts where there is no ambiguity, the $ may be omitted: for
    example, the user interface shows register and memory contents as
    hexadecimal without the leading $.
...
leftmost, or most significant).  The condition code bits that have
specific meanings are called *flags*.
...
- Index: Each flag has an index which gives its bit position in
  the condition code.  Bits are numbered from right to left, starting
  with 0.
...
- Hex: The value of the condition code word assuming the given flag is
  ...
  condition code register will be the sum of the hex values of the
  true flags.  For example, suppose the words 00a3 is compared with
  00c5.  The comparison result is L (less than for naturals) and also
  l (less than for integers).  So the condition code will contain
  0008 + 0010 = 0018.
...
|    10 | f      | logicc function result | 0400 |
|    11 |        | reserved               | 0800 |
|    12 |        | reserved               | 1000 |
|    13 |        | reserved               | 2000 |
|    14 |        | reserved               | 4000 |
...
#+END_EXPORT
|-------+--------+-------+---------------------+-----------|
| Index | symbol | value | relation            | type      |
|-------+--------+-------+---------------------+-----------|
|     0 | g      |  0001 | greater, positive   | integer   |
|     1 | G      |  0002 | greater, positive   | natural   |
|     2 | E      |  0004 | equal, zero         | all types |
|     3 | L      |  0008 | less than           | natural   |
|     4 | l      |  0010 | less than, negative | integer   |
|     5 | v      |  0020 | overflow            | integer   |
|     6 | V      |  0040 | overflow            | natural   |
|     7 | C      |  0080 | carry               | natural   |
|     8 | S      |  0100 | stack overflow      |           |
|     9 | s      |  0200 | stack underflow     |           |
|    10 | f      |  0400 | logicc result       | bit       |
|-------+--------+-------+---------------------+-----------|
# #+TBLFM: $4=, zero
#+BEGIN_EXPORT html
</div>
...
| effect             | mem[reg[f]+gh] := reg[d]     |
|                    | mem[reg[f]+gh+1] := reg[d+1] |
|                    | ...                          |
|                    | mem[reg[f]+gh+e-d] := reg[e] |
|--------------------+------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
| effect             | reg[d] := mem[reg[f]+gh]=    |
|                    | reg[d+1] :- mem[reg[f]+gh+1] |
|                    | ...                          |
|                    | reg[e] := mem[reg[f]+gh+e-d] |
|--------------------+------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
