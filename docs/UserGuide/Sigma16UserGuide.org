...
Sigma16 provides pseudoinstructions for the commonest logic
operations.  Each instruction takes one or two operands, calculates
the result, and places the result in the destination register.

These are "bitwise" operations: bit $i$ of the result is
the logical /and/ of bit $i$ of each operand, and does not depend on
bits at any other index.

- invw R1,R2 inverts the bits of R2 and places the result in R1
- andw R1,R2,R3 calculates the logical /and/ of the operands R2 and R3,
  and places the result in the destination register R1.
- orw R1,R2,R3 calculates the logical /inclusive or/ of the operands
  R2 and R3, and places the result in the destination register R1.
- xorw R1,R2,R3 calculates the logical /exclusive or/ of the operands
  R2 and R3, and places the result in the destination register R1.

 Similarly, there is
=orw= to calculate the logical /inclusive or/ of two words, and =xorw=
for /exclusive or/.  To invert all the bits in a word, use =invw=.

The following examples are in Examples/ Standard/ Logic.asm.txt
...
The bit logic instruction =logicb= extracts the operand bits from
registers, calculate the result using a supplied function, and put the
bit back into a word, leaving the rest of the word unchanged.  The
word logic instructions update every bit in the destination.  The bit
logic instructions update only one bit in the destination, enabling
you to keep many Boolean variables in just one register.
...
+--------------+----+----------+------------------------+---------+
| add Rd,Ra,Rb |  0 | $0 d a b | R[d] := R[a] + R[g]    | gGElvVC |
+--------------+----+----------+------------------------+---------+
...
| Assembly            | Op   | Code     | Effect               | CC |
+---------------------+------+----------+----------------------+----+
| logicw Rd,Re,Rf,h   | e,00 |          | Rd := Re (lut g) Rf  |    |
+---------------------+------+----------+----------------------+----+
| logicb Rd,e,Rf,g,h  | e,02 |          | Rd.e := Rd.f #h Rd.g |    |
+---------------------+------+----------+----------------------+----+
| shiftl Rd,Re,h      | e,03 |          | Rd := Re shl h       |    |
+---------------------+------+----------+----------------------+----+
| shiftr Rd,Re,h      | e,04 |          | Rd := Re shr h       |    |
+---------------------+------+----------+----------------------+----+
| extract Rd,f,g,Re,h | e,05 |          | Rd.f~g :=            |    |
+---------------------+------+----------+----------------------+----+
| extract Rd,f,g,Re,h | e,05 |          | Re.h~h+g-h+1         |    |
+---------------------+------+----------+----------------------+----+
| extracti Rd,E,L,M,N | e,06 |          | Rd := ex Rd di       |    |
+---------------------+------+----------+----------------------+----+
| extracti Rd,E,L,M,N | e,06 |          | sz Rs si             |    |
+---------------------+------+----------+----------------------+----+
| push Rd,E,F         | e,07 |          | M[ea] := Rd, Rb++    |    |
+---------------------+------+----------+----------------------+----+
| pop D,E,F           | e,08 |          | Rb--, Rd := M[ea]    |    |
+---------------------+------+----------+----------------------+----+
| top D,E,F           | e,09 |          | Rb--, Rd := M[ea]    |    |
+---------------------+------+----------+----------------------+----+
| save Rd,e,gh[Rf]    | e,0a |          | M[ea] := R1,         |    |
|                     |      |          | ..., M[ea+d-1] := Rd |    |
|                     |      |          | ..., M[ea+d-1] := Rd |    |
+---------------------+------+----------+----------------------+----+
| restore Rd,e,gh[Rf] | e,0b |          | R1 := M[ea],         |    |
|                     |      |          | ..., Rd := M[ea+d-1] |    |
+---------------------+------+----------+----------------------+----+
| brc0 Rd,e,fgh       | e,0c | $e d $0c | if R[d].e=0          |    |
|                     |      | e fgh    | then pc:=pc+fgh      |    |
+---------------------+------+----------+----------------------+----+
| brc1 Rd,e,fgh       | e,0d | $e d $0d | if R[d].e=1          |    |
| Rd,e,fgh            | e,0d | e fgh    | then pc:=pc+fgh      |    |
+---------------------+------+----------+----------------------+----+
| brz Rd,efgh         | e,0e | $e d $0e | if R[d]=0000         |    |
|                     |      | efgh     | then pc:=pc+efgh     |    |
+---------------------+------+----------+----------------------+----+
| brnz Rd,efgh        | e,0f | $e d $0f | if R[d]!=0000        |    |
|                     |      | efgh     | then pc:=pc+efgh     |    |
+---------------------+------+----------+----------------------+----+
| dispatch d,x        | e,10 |          | if Rd!=0             |    |
|                     |      |          | then pc += 2-fgh     |    |
+---------------------+------+----------+----------------------+----+
| getctl Rd,c         | e,11 |          | Rd := Sc             |    |
|                     |      |          | privileged           |    |
+---------------------+------+----------+----------------------+----+
| putctl Rd,c         | e,12 |          | Sc := Rd             |    |
|                     |      |          | privileged           |    |
+---------------------+------+----------+----------------------+----+
| resume              | e,13 |          | pc := ipc,           |    |
|                     |      |          | status := istatus    |    |
