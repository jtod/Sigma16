...
This section gives the complete instruction set.  There are
separate tables for each instruction format.  Each instruction
...
2. Indication if the instruction is Core, followed by the full
   expanded operation code and the language statement format.
3. An example assembly language statement, with specific operands,
   and the corresponding machine language.
...
# S32 hidden
<<xadd-t>> <<xsub-t>> <<xmul-t>> <<xdiv-t>> <<xcmp-t>> <<xaddc-t>>
<<xmuln-t>> <<xdivn-t>> <<xjump-t>> <<xjal-t>> <<xjumpz-t>>
<<xjumpnz-t>> <<xsave-t>> <<xrestore-t>> <<xpush-t>> <<xpop-t>>
<<xtop-t>>
