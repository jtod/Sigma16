...
# Copyright 2010-2026 John T. O'Donnell.  All rights reserved.
# Source repository: https://github.com/jtod/Sigma16

# License: GNU GPL Version 3, available in Sigma16/LICENSE.txt and
# online at <https://www.gnu.org/licenses/>.  Further information
# is in Sigma16/README.org.

# This file is part of Sigma16.  Sigma16 is free software: you can
# redistribute it and/or modify it under the terms of the GNU
# General Public License, Version 3, as published by the Free
# Software Foundation.  Sigma16 is distributed in the hope that it
...
# If you are doing a project with Sigma16, or experimenting with
# it, please see the section "* Projects with Sigma16"

# ----------------------------------------------------------------
# Quick reference: how to build this document (requires emacs org)
# ----------------------------------------------------------------
...
# S16version.  {{{S16version()}}} is used to define the DATE
# attribute.  It is defined by docs/VersionMacro.org, which is
# generated automatically by make SetVersion.  It gives the
# version number and date, and is defined by make SetVersion

# The following  is a benign redefinition to prevent error/abort on
...
# Macros for importing figures.  Macro definitions must be
# entirely on one line, so these definitions are rediculously
# long.  To import ./figures/tikz/RRR.{svg,pdf}:

#   {{{BeginGraphics}}}
#   {{{GraphicsCmd}}}RRR{{{GraphicsExt}}}
#   {{{EndGraphics}}}

#+MACRO: BeginGraphics (eval (if (eq org-export-current-backend 'html) "#+BEGIN_EXPORT html" "#+BEGIN_EXPORT latex"))
#+MACRO: EndGraphics (eval (if (eq org-export-current-backend 'html) "#+END_EXPORT" "#+END_EXPORT"))
#+MACRO: GraphicsCmd (eval (if (eq org-export-current-backend 'html) "<img src=\"./figures/tikz/" "\\includegraphics{./figures/tikz/"))
#+MACRO: GraphicsExt (eval (if (eq org-export-current-backend 'html) ".svg\">" ".pdf}"))
...
# ----------------------------------------------------------------
# Typesetting and editing source
# ----------------------------------------------------------------
...
# --- Using Sigma16 style for standalone pdf
#+LATEX_CLASS: article
#+LATEX_CLASS_OPTIONS: [letterpaper,12pt]
#+LATEX_HEADER: \usepackage{../Sigma16}
#+LATEX_HEADER: \usepackage{svg}

# --- for using compsys book style (disabled)
#   #+LATEX_CLASS: svmono
#   #+LATEX_CLASS_OPTIONS: []
#   #+LATEX_HEADER: \usepackage{../compsys}
...
# Each instruction has a level 4 section (subsubsubsection, or
# paragraph), describing the instruction.  There is an anchor for
# the section e.g. <<add-d>>.  Each instruction also has an entry
# in the Table of Instructions, and there is an anchor <<add-t>>
# for the table entry.

# <<add-d>> is for description; <<add-t>> is for table entry
...
implemented, and written by John O'Donnell.  Copyright (C)
2010-2026 John T. O'Donnell.  Contact email://
...
Sigma16 illustrates the basic ideas of computer systems while
avoiding unnecessary complexity. Most commercial computers that
achieve success in the marketplace evolve through updates and
extensions, and eventually become encrusted with complications
that help support backward compatibility but that lead to
complexity.
...
- *store* copies a word from a register into a memory location.
  Suppose R3 contains the result of some calculations, and we want
  to put it back into memory in a variable named ~result~.  This is
  achieved by ~store R3,result[R0]~.
...
- Go to the *Processor* page.  Click *Boot*, then *Step*
  repeatedly and watch the effect of each instruction by observing
  how the registers and memory are changed.
...
We also need to specify three registers: the destination and two
source operands.  There are 16 registers, so a specific one can be
specified by 4 bits.  Thus the requirements for an RRR instruction
are 4 fields, each containing 4 bits, for a total of 16 bits.  An
RRR instruction exactly fills one word.
...
The aim of this tutorial is to figure out what the program does,
how it works, and to do so for several specific values of y.  A
solution is given below, but please try first to work it out for
yourself.
...
2. Execute the program manually, with paper and pencil but
   ...
   runs.
3. Assemble the program and boot it in the processor.  Run the
   program on the emulator and check whether your execution was
   correct.  Be sure to single-step through the program; don't
   just run it at full speed.  Before executing each instruction,
   think about what it will do.  Then click Step and look at the
...
5. When y=-5424 the program goes into an infinite loop. ~R1=ead0~,
   which is the hexadecimal representation of -5424.  R2=f003,
   R3=3, and R4=4.  What started out as the ~add~ instruction at
   ~x~ has been transformed into ~jump 7[R0]~, comprising the word
   at ~x~ (whose value is ~f003~) and the following word (which is
   ~0007~).  This ~jump~ instruction goes back to the first ~lea~
   instruction, and the program runs for ever: ~lea~, ~lea~,
   ~jump~, ~lea~, ~lea~, ~jump~, $\ldots$.
...
| div R1,R2,R3        | R1 := R2 / R3, R15 := remainder   |
| cmp R1,R2           | compare R1,R2, set R15            |
| trap R1,R2,R3       | if R1=0 then halt                 |
|                     |                                   |
| lea R1,const[R12]   | R1 := const+R1                    |
| load R1,const[R12]  | R1 := Mem[const+R12]              |
| store R1,const[R12] | M[const+R12] := R1                |
| jump const[R12]     | goto const+R12                    |
| jal R11,const[R12]  | R11 := pc, goto const+R12         |
|---------------------+-----------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: Core Instructions, normally use pseudoinstruction instead
|---------------------+-----------------------------------|
| jumpc0 3,const[R12] | if R15.3=0 then goto M[const+R12] |
| jumpc1 3,const[R12] | if R15.3=1 then goto M[const+R12] |
|---------------------+-----------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: Core Pseudoinstructions
|-----------------+--------------------------|
| jumplt loop[R0] | if $<$ then goto loop    |
| jumple loop[R0] | if $\leq$ then goto loop |
| jumpeq loop[R0] | if $=$ then goto loop    |
| jumpne loop[R0] | if $\neq$ then goto loop |
| jumpge loop[R0] | if $\geq$ then goto loop |
| jumpgt loop[R0] | if $>$ then goto loop    |
|-----------------+--------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
support logic and bit manipulation.  There are also instructions
that simplify functions and procedures.  The architecture provides
additional registers that control system functions, such as
interrupts.  These features support systems programming, including
emulators, interpreters, and operating system kernels.  There are
assembler directives that simplify large scale programming,
modules, linking, and relocation.
...
[[shiftl-t][shiftl]] and [[shiftr-t][shiftr]].
They shift the word in Re
...
Sigma16 provides an instruction [[extract-t][extract]] which solves this
...
The [[extract-t][extract]] instruction modifies only the specified bits in
...
Here is where it goes bad.  The begin example above works, the
macro below doesn't work. bad
...
Problematical.  R0 always contains the constant 0.  It is legal to
perform an instruction that attempts to load some other value into
R0, but the register will still contain 0 after executing such an
instruction.  Such an instruction will have no lasting effect.
...
This subsubsection is about the cc
...
* Sigma16 Instruction set

The Sigma16 system contains a full 16-bit Standard architecture
which supports advanced programming techniques.  This section
describes the full Standard architecture

A subset of the Standard architecture is called the Core
architecture.  This is sufficient for beginners and for a short
course on computer systems.  The tutorials are separated into Core
and Standard parts.

There are 12 Core instructions:
- [[add-d][add]], [[sub-d][sub]], [[mul-d][mul]], [[div-d][div]]
- [[lea-d][lea]], [[load-d][load]], [[store-d][store]]
- [[cmp-d][cmp]], [[jump-d][jump]], [[jumpc0-d][jumpc0]],
  [[jumpc1-d][jumpc1]]
- [[trap-d][trap]]
There are also 6 Core pseudoinstructions to make conditionals
easier:

- [[jumplt-d][jumplt]], [[jumple-d][jumple]],
  [[jumpeq-d][jumpeq]], [[jumpne-d][jumpne]],
  [[jumpge-d][jumpge]], [[jumpgt-d][jumpgt]]

The system hardware doesn't treat the Core differently; the
hardware doesn't know or care whether an instruction is part of
the Core.  The purpose of the Core subset is to have a combination
of tutorials, documentation, and examples that are self contained,
and don't require knowing about the reset of the system.

All the other instructions described below are in the Standard
architecture.
...
We will refer both to fields of instructions, and the actual hex
values of fields.  The notation is case sensitive: a is not the
same as A.
- Upper case letters are instruction field names.
- Lower case letters are hex digits.
Thus A is a fields name, while a is the value 1010 and any field
might have that value.
...
- *RX*
  - There are 16 RX instructions: f,0--f
  - 2 words
  - operation: f,B
...
- *EXP*
  - There are up to 192 EXP instructions: e,00--bf
  - 2 words
  - operation: e,AB
...
In assembly language, the effective address is written ~disp[Ra]~.
For example, ~3[R8]~ means "3 + contents of R8".  The displacement
can be written as either decimal or hex, whichever is more
convenient.  Thus ~$20b3[R2]~ means the effective address has
displacement ~$20b3~ and the index register is R2.
...
- The first word has the same op and d fields as RRR and RX.  The
  op field contains 14 (hex e), which indicates that the
  instruction is EXP format.  The a and b fields are treated as
  one 8-bit natural number, which is the secondary operation code.
  This provides for the possibility of up to 256 EXP format
  instructions, which enables new experimental instructions to be
  defined.
...
{{{BeginGraphics}}}
{{{GraphicsCmd}}}EXP{{{GraphicsExt}}}
{{{EndGraphics}}}

- E  (bits 15-12) 4-bit operand
- F  (bits 11-8)  4-bit operand
- G  (bits 7-4)   4-bit operand
- J  (bits 3-0)   4-bit operand

** Arithmetic

*** Addition and subtraction

- Instructions: [[add-d][add]], [[sub-d][sub]]

The addition and subtraction instructions can be used on both
integers and natural numbers.  If both operands are integers
(two's complement) then the result is also an integer.  If both
operands are natural numbers (binary) then the result is also a
natural number.  The condition code has separate flags to indicate
overflow for integers and natural numbers.

<<add-d>>
**** add

The instruction [[add-t][_add_]]  has operands ~Ra~ and ~Rb~
and destination ~Rd~.  It fetches the operands ~Ra~ and ~Rb~,
...
The [[sub-t][_sub_]] instruction is similar to add; it calculates R2-R3
and places the result in R1.  The effect on the condition code is
the same as for add.
...
- Instructions: [[mul-d][mul]], [[div-d][div]]

The mul and div instructions operate on integer operands.  For
multiplication and division of natural numbers, use the [[muln-d][muln]] and
[[divn-d][divn]].

<<mul-d>>
**** mul

The [[mul-t][_mul_]] instruction multiplies two integers.  mul Rd,Ra,Rb
calculates the integer (two's complement) product of the operands
Ra and Rb, and places the result in the destination register Rd.
The mul instruction does not produce the natural (binary) product.
...
The [[div-t][_div_]] instruction performs a division and produces the
...
The =div= instruction produces two results: the quotient and the
remainder.  The quotient is loaded into the destination register
Rd, and the remainder is loaded into R15.
...
- Instruction: [[cmp-d][cmp]]
  
<<cmp-d>>
**** cmp

The [[cmp-t][_cmp_]] instruction =cmp Ra,Rb= compares the values
in the operand registers =Ra= and =Rb=, and then sets flags in the
condition code (R15) to indicate the result. The notation R15.i
means bit /i/ in R15; thus R15.0 is the leftmost bit of R15.  The
instruction performs both natural number comparison (binary) and
integer comparison (two's complement).  The resulting flags are
...
*** Extensible natural arithmetic

- Instructions: [[addc-d][addc]], [[muln-d][muln]], [[divn-d][divn]]
  
<<addc-d>>
**** addc

The [[addc-t][_addc_]] instruction performs a binary addition with
carry propagation.  It adds the two operand registers and the
carry bit in the condition code register, R15.  The sum is loaded
into the destination register Rd and the carry output is written
back into the carry bit, overwriting its previous value.  Overflow
is not possible with this instruction.

<<muln-d>>
**** muln

The [[muln-t][_muln_]] instruction calculates the product of two
natural numbers in Ra and Rb.  The result is 32 bits; the leftmost
16 bits (the most significant part) is loaded into R15, and the
rightmost 16 bits (the least significant part) is loaded into Rd.
If Rd is R15, the most significant part is discarded.
...
The [[divn-t][_divn_]] instruction divides two natural numbers:
dividend / divisor.  All the numbers -- numerator, denominator,
quotient, remainder -- are natural numbers represented in binary.
...
- Instruction: [[lea-d][lea]]
  
<<lea-d>>
**** lea

The [[lea-t][lea]] instruction stands for "load effective
address".   *lea Rd,disp[Ra]*
calculates the effective address of the operand disp[Ra] and
places the result in the destination register Rd.  The effective
address is the binary sum disp+Ra.
...
*** Copying between register and memory

- Instructions: [[load-d][load]], [[store-d][store]]

<<load-d>>
**** load

The [[load-t][load]] instruction *load Rd,disp[Rx]* calculates the
effective address of the operand disp[Rx] and copies the word in
memory at the effective address into the destination register Rd.
The effective address is the binary sum disp+Rx.
...
The [[store-t][store]] instruction *store Rd,disp[Rx]* calculates
the effective address of the operand disp[Rx].  The value of the
destination register Rd is stored into memory at the effective
address.
...
- Instructions: [[push-d][push]], [[pop-d][pop]], [[top-d][top]]
...
The [[push-t][push]] instruction pushes an element onto a stack.  It is
...
The [[pop-t][pop]] instruction removes an element onto a stack and
...
The [[top-t][top]] instruction returns the top element on a stack but
...
- Instructions: [[save-d][save]], [[restore-d][restore]]
...
The [[save-t][save]] instruction performs a sequence of stores, which
...
This [[restore-t][restore]] instruction has the same operands as
=save=, and it performs a sequence of loads corresponding to the
...
#+BEGIN_SRC Sigma16 exports: code
; store R3-R9 starting at 6[R13]  
   restore R3,R9,6[R13]  ; e30b 9d06
#+END_SRC
...
#+BEGIN_SRC Sigma16 exports: code
; load R3-R5 starting at 6[R13]
   restore R3,R5,6[R13]  ; e30c 5d06
#+END_SRC
...
- Instructions
  - Unconditional: [[jump-d][jump]]
  - Conditional: [[jumpc0-d][jumpc0]], [[jumpc1-d][jumpc1]],
    [[jumpz-d][jumpz]], [[jumpnz-d][jumpnz]]
- Pseudoinstructions:

- [[jumplt-d][jumplt]], [[jumple-d][jumple]], [[jumpeq-d][jumpeq]],
  [[jumpne-d][jumpne]], [[jumpge-d][jumpge]], [[jumpgt-d][jumpgt]]

<<jump-d>>
**** jump

A [[jump-t][jump]] is a transfer control to another address, rather
than to the following instruction. The destination address is
specified as the effective address in the jump instruction.  A
jump can be used to implement a goto statement.
...
A conditional jump goes to a destination if a condition is true;
otherwise it does nothing.  The condition can either be a specified bit
in the condition code is 0 (for jumpc0) or 1 (for jumpc1), or the value

<<jumpc0-d>>
**** jumpc0

[[jumpc0-t][jumpc0]] jumps if a specified bit in R15 is 0.  The first
operand is the index of that bit.  Don't specify R15; the
condition code bit is always in R15.  The index of the condition
bit must be specified in the d field of the instruction.  Since
this is an index, not a register, it's written as a number.
...
[[jumpc1-t][jumpc1]] is similar to jumpc0: it jumps if the specified bit is 1.
...
The [[jumpz-t][jumpz]] instruction jumps to the destination if a specified
register is 0 (i.e. all the bits in that register are 0).

[[jumpnz-t][jumpnz]] is similar, but it jumps if the register does
not contain 0.
...
<<jumplt-d>> <<jumple-d>> <<jumpeq-d>> <<jumpne-d>> <<jumpge-d>>
<<jumpgt-d>>
**** Conditional jump after comparisons

There are pseudoinstructions for common cases of conditional
jumps, after a comparison between two integers has been made.

| Pseudo          | Real instruction  | Meaning        |
|-----------------+-------------------+----------------|
| jumplt disp[Ra] | jumpc1 k,disp[Ra] | jump if $<$    |
| jumple disp[Ra] | jumpc0 k,disp[Ra] | jump if $\leq$ |
| jumpeq disp[Ra] | jumpc1 k,disp[Ra] | jump if $=$    |
| jumpne disp[Ra] | jumpc0 k,disp[Ra] | jump if $\neq$ |
| jumpge disp[Ra] | jumpc0 k,disp[Ra] | jump if $>$    |
| jumpgt disp[Ra] | jumpc1 k,disp[Ra] | jump if $\geq$ |

These pseudoinstructions cover only the possible conditionals
based on 3 of the condition code bits, but the condition code
contains many more bits (for example, comparisons between natural
numbers, presence of overflow, and more).  A conditional jump
based on these other bits requires using the appropriate jumpc0 or
jumpc1.
...
The [[jal-t][jal]] instruction jumps to a destination, and also
records the location it jumped from.  This is needed for most
...
[[brc0-t][brc0]] and [[brc1-t][brc1]] There are two conditional
branch instructions that perform a branch based on the value of a
bit in a register.  For example, brc0 R7,3,xyz means "if bit 3 of
R7 is 0 then go to xyz".
...
These [[brz-t][brz]] and [[brnz-t][brnz]] instructions perform a branch if a specified register
...
The [[dispatch-t][dispatch]] instruction implements a branch table.  It
provides an efficient way to implement a case statement using
a binary code.

Suppose =code= is a variable that contains a natural number, such
that $0 \leq code < 8$.  The following set of instructions will
branch to a specific destination depending on the value of the
code.
...
- =logicf= performs bitwise logic on fields within two operand
  registers.  The fields must have the same indices in the two
  registers.  The result overwrites the field in the first
  operand.
...
The [[logicf-t][logicf]] instruction performs a bitwise logic
operation on fields within two words.  The fields may be in
different registers, but each field must be at the same position:
their start and end indices must be the same.  Each bit of the
result is obtained by performing a specified logic function on the
corresponding bits of the two fields.  The result overwrites the
field in the first operand register. The function $h$ is specified
as described above; for example, 6 is the code for exclusive or.
...
#+BEGIN_EXAMPLE
; logicf  Rd,Re,f,g,h
;   Rd = first operand  and destination
;   Re = second operand register
;   f = index of rightmost bit in field in Rd and Re
;   g = index of leftmost bit in field in Rd and Re
;   h = logic function
...
A field can comprise all the bits in a word, so logic on words can
be performed using the field logic instruction.  Several special
cases of =logicf= appear frequently.  There are special
pseudoinstructions that simplify these special cases by allowing
some of the operands to be omitted.
...
The bit logic instruction [[logicb-t][logicb]] specifies two
operand bits, which may be in any register at any index.  It
applies a specified logic function to these bits and puts the
result bit back into the first operand, overwriting the previous
value of that bit.  The other bits in the first operand register
remain unchanged.  As this instruction modifies only one bit in
the first operand, it enables you to keep many Boolean variables
...
#+BEGIN_SRC Sigma16 exports: code
   ...
   logicb R2,R7,9,13,xor ; R2 := 0b00
                         ; (R2.9 := R2.9 xor R7.13 = 0)
#+END_SRC
...
<<shiftl-d>>
The instruction [[shiftl-t][shiftl]] =Rd,Re,h= shifts the value in the
...
<<shiftr-d>>
The instruction [[shiftr-t][shiftr]] =Rd,Re,h= shifts the value in the
...
The [[extract-t][extract]] instruction copies a field within a register to
...
- Instructions: [[trap-d][trap]], [[getctl-d][getctl]],
  [[putctl-d][putctl]]

<<trap-d>>
**** trap

The [[trap-t][_trap_]] instruction provides a way for a user program
to make a request to the operating system.
  
<<getctl-d>>
**** getctl

The [[getctl-t][getctl]] instruction

<<putctl-d>>
**** putctl

The [[putctl-t][putctl]] instruction

*** Mutual exclusion

- Instruction [[testset-d][testset]]

<<testset-d>>
**** testset

The [[testset-t][testset]] instruction supports mutual exclusion.
...
- Instruction: [[resume-d][resume]]

<<resume-d>>
**** resume

The [[resume-t][resume]] instruction

*** Timer

- Instructions [[timeron-d][timeron]], [[timeroff-d][timeroff]]

<<timeron-d>>
<<timeroff-d>>
**** timeron, timeroff

The [[timeron-t][timeron]] instruction

The [[timeroff-t][timeroff]] instruction
...
Sigma32 extends the Sigma16 architecture to support 32-bit words.  

Most 16-bit instructions have a 32-bit counterpart: add/xadd,
sub/xsub, and so on.  These instructions are exceptions, as they
have no corresponding 32 bit instruction.
- testset - no need for 32 bit operands
- dispatch - no need for 32-bit operands
- getctl, putctl, resume - can use 32 bit registers; they just
  operate on 32 bits
- timeron, timeroff -  want to reconsider these anyway.  Use just
  one instruction with an operand
- jump (should it exist for 32?  can use xjumpc0 R0,...)
- br family

The entire system, including the app which runs
...
** Representation of 32-hit instructions
*** S32 RX instructions
...
*** S32 EXP3 instructions

EXP3 is a subformat of EXP.
...
|------------------------------------------------------+-----------+-------------------------------|
| <<xtrap-t>> [[xtrap-d][xtrap]] Rd,Re,Rf              |           | Rd.f~g :=                     |
| [e,28] iEXP/aRRR *S32*                               |           |                               |
| xtrap R4,R13,R8                                      | e900 37f2 |                               |
...
This section gives the complete instruction set, including Core,
Standard, and Sigma32.  If an instruction is not marked as Core or
S32, it is a standard 16-bit instruction.  There are separate
tables for RRR, RX, EXP, and EXP3 instructions.  Each instruction
entry contains several lines:

1. General form of assembly language statement, general form of
   the machine language, and the instruction's effect.  Field
   names are upper case and specific hex numbers are lower case.
   Thus the general form of the load instruction is load
   RD,DISP[RA], and its object code is fDA1 DISP (the OP field is
   the hex number f, and the values of the D and A fields are
   specified in the assembly instruction).
2. Indication if the instruction is Core or S32, followed by the ?????
   blabla ref    expanding opcode full expanded operation code and the
    assembly-format assembly language statement format]
3. An example assembly language statement, with specific operands,
   and the corresponding machine language.  These examples also
   appear in Examples/Sigma32/testasm.asm.txt.

<<RRR instruction reference>>
** RRR instruction reference

#+BEGIN_EXPORT html
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: RRR instructions
#+ATTR_LATEX: :environment longtable :align l|l|l
| *Assembly*                                  | *Code* | *Effect*                  |
|---------------------------------------------+--------+---------------------------|
| *Core* *<<add-t>>[[add-d][_add_]] RD,RA,RB* | 0DAB   | R[d] := R[a]+R[g]         |
| [0] iRRR/aRRR                               |        | R15 := (cc flags)         |
| add R8,R12,R5                               | 08c5   |                           |
|---------------------------------------------+--------+---------------------------|
| *Core* <<sub-t>>[[sub-d][_sub_]] RD,RA,RB   | 1DAB   | R[d] := R[a]-R[b]         |
| [1] iRRR/aRRR                               |        | R15 := (cc flags)         |
| sub R8,R12,R5                               | 18c5   |                           |
|---------------------------------------------+--------+---------------------------|
| *Core* <<mul-t>> [[mul-d][_mul_]] D,A,B     | 2DAB   | R[d] := R[a]*R[b]         |
| [2] iRRR/aRRR                               |        | R15 := (cc flags)         |
| mul R8,R12,R5                               | 28c5   |                           |
|---------------------------------------------+--------+---------------------------|
| *Core* <<div-t>> [[div-d][_div_]] D,A,B     | 3DAB   | R[d] := Ra / Rb,          |
| [3] iRRR/aRRR                               |        | R[15] := Ra % Rb          |
| div R8,R12,R5                               | 38c5   |                           |
|---------------------------------------------+--------+---------------------------|
| *Core* <<cmp-t>> [[cmp-d][_cmp_]] A,B       | 40AB   | R[15] := R[a] cp R[b]     |
| [4] iRRR/aRR                                |        | R15 := (cc flags)         |
| cmp R12,R5                                  | 48c5   |                           |
|---------------------------------------------+--------+---------------------------|
| <<addc-t>> [[addc-d][_addc_]] D,A,B         | 5DAB   | Rd := Ra + Rb + R15.C)    |
| [5] iRRR/aRRR                               |        | R15 := (cc flags)         |
| addc R8,R12,R5                              | 58c5   |                           |
|---------------------------------------------+--------+---------------------------|
| <<muln-t>> [[muln-d][_muln_]] D,A,B         | 6DAB   | Rd := Ra :*: Rb, R15 :=   |
| [6] iRRR/aRRR                               |        |                           |
| muln R8,R12,R5                              | 68c5   |                           |
|---------------------------------------------+--------+---------------------------|
| <<divn-t>> [[divn-d][_divn_]] D,A,B         | 7DAB   | R15++Rd := R15++Ra :/: Rb |
| [7] iRRR/aRRR                               |        |                           |
| divn R8,R12,R5                              | 78c5   |                           |
|---------------------------------------------+--------+---------------------------|
| [8]                                         | 8___   | nop -- available          |
|---------------------------------------------+--------+---------------------------|
| [9]                                         | 9___   | nop -- available          |
|---------------------------------------------+--------+---------------------------|
| [a]                                         | a___   | nop -- available          |
|---------------------------------------------+--------+---------------------------|
| [b]                                         | b___   | nop -- available          |
|---------------------------------------------+--------+---------------------------|
| *Core* <<trap-t>>[[trap-d][_trap_]] D,A,B   | cDAB   | interrupt                 |
| [c] iRRR/aRRR                               |        |                           |
| trap R8,R12,R5                              | c8c5   |                           |
|---------------------------------------------+--------+---------------------------|
| [d]                                         | d___   | reserved -- available     |
|---------------------------------------------+--------+---------------------------|
| [e,AB] *EXP*                                | eDAB   | escape to EXP             |
|                                             |        | secondary opcode = AB     |
|---------------------------------------------+--------+---------------------------|
| [f,B] *RX*                                  | fDAB   | escape to RX              |
...
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: RX instructions
#+ATTR_LATEX: :environment longtable :align l|l|l
| *Assembly*                                            | *Code*    | *Effect*                       |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<lea-t>>[[lea-d][_lea_]] RD,DISP[RA]          | fDA0 DISP | Rd := disp+Ra                  |
| [f,0] iRX/aRX                                         |           |                                |
| lea R4,$0e3a[R14]                                     | f4e0 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<load-t>>[[load-d][_load_]] RD,DISP[RA]       | fDA1 DISP | Rd := M[disp+Ra]               |
| [f,1] iRX/aRX*                                        |           |                                |
| load R4,$0e3a[R14]                                    | f4e1 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<store-t>>[[store-d][_store_]] RD,DISP[RA]    | fDA2 DISP | M[disp+Ra] := Rd               |
| [f,2[ iRX/aRX                                         |           |                                |
| store R4,$0e3a[R14]                                   | f4e2 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<jump-t>>[[jump-d][_jump_]] DISP[RA]          | fDA3 DISP | pc := disp+Ra                  |
| [f,3] iRX/aX                                          |           |                                |
| jump $0e3a[R14]                                       | f0e3 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<jumpc0-t>>[[jumpc0-d][_jumpc0_]] D,DISP[RA]  | fDA4 DISP | *if* R15.d=0                   |
| [f,4] iRX/akX                                         |           | *then* pc := disp+Ra           |
| jumpc0 4,$0e3a[R14]                                   | f4e4 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<jumpc1-t>>[[jumpc1-d][_jumpc1_]] D,DISP[RA]  | fDA5 DISP | if R15.d=1                     |
| [f,5] iRX/akX                                         |           | then pc := disp+Ra             |
| jumpc1 4,$0e3a[R14]                                   | f4e5 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *Core* <<jal-t>>[[jal-d][_jal_]] RD,DISP[RA]          | fDA6 DISP | Rd:=pc,                        |
| [f,6] iRX/aRX                                         |           | pc:=disp+Ra                    |
| jal R4,$0e3a[R14]                                     | f4e6 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| <<jumpz-t>> [[jumpz-d][_jumpz_]] RD,DISP[RA]          | fDA7 DISP | if R15.d=0                     |
| [f,7] iRX/aRX                                         |           | then pc := disp+Ra             |
| jumpz R4,$0e3a[R14]                                   | f4e7 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| <<jumpnz-t>> [[jumpnz-d][_jumpnz_]] RD,DISP[RA]       | fDA8 DISP | if R15.d=1                     |
| [f,8] iRX/aRX                                         |           | then pc := disp+Ra             |
| jumpnz R4,$0e3a[R14]                                  | f4e8 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| <<testset-t>> [[testset-d][_testset_]] RD,DISP[RA]    | fDA9 DISP | Rd:=M[disp+Ra],                |
| [f,9] iRX/aRX                                         |           | M[disp+Ra]:=0001               |
| testset R4,$0e3a[R14]                                 | f4e9 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
|-------------------------------------------------------+-----------+--------------------------------|
| *S32* <<xlea-t>> [[xlea-d][_xlea_]] RD,DISP[RA]       | fDAa DISP | R_d,R_{d+1} :=                 |
| [f,a] iRX/aRX                                         |           | disp \oplus R_a                |
| xlea R4,$0e3a[R14]                                    | f4ea 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *S32* <<xload-t>> [[xload-d][_xload_]] RD,DISP[RA]    | fDAb DISP | R_d,R_{d+1} :=                 |
| [f,b] iRX/aRX                                         |           | M[ea],mem[ea+1]                |
| xload R4,$0e3a[R14]                                   | f4eb 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
| *S32* <<xstore-t>> [[xstore-d][_xstore_]] RD,DISP[RA] | fDAc DISP | M[ea],M[ea+1] := R_d,R_{d+1}$' |
| [f,c] iRX/aRX                                         |           |                                |
| xstore R4,$0e3a[R14]                                  | f4ec 0e3a |                                |
|-------------------------------------------------------+-----------+--------------------------------|
|                                                       | fDAd DISP | nop -- available               |
| [f,d] iRX/                                            |           | for experimentation            |
|-------------------------------------------------------+-----------+--------------------------------|
|                                                       | fDAe DISP | nop -- available               |
| [f,e] iRX/                                            |           | for experimentation            |
|-------------------------------------------------------+-----------+--------------------------------|
|                                                       | fDAf DISP | nop -- available               |
| [f,f] iRX/                                            |           | for experimentation            |
|-------------------------------------------------------+-----------+--------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
<<EXP instruction reference>>
** EXP instructions

#+BEGIN_EXPORT html
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: EXP instruction reference
#+ATTR_LATEX: :environment longtable :align l|c|l
|------------------------------------------------------------+-----------+-------------------------------|
| *Assembly*                                                 | *Code*    | *Effect*                      |
|------------------------------------------------------------+-----------+-------------------------------|
| <<logicf-t>> [[logicf-d][_logicf_]] RD,RE,F,G,H            | eD00 EFGH | Rd.f:g :=                     |
| [e,00] iEXP/aRRkkk                                         |           | h(Rd.f:g,Re.f:g)              |
| logicf R9,R3,7,15,2                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<logicb-t>> [[logicb-d][_logicb_]] Rd,Re,f,g,h            | eD01 EFGH | Rd.f := h(Rd.f,Re.g)          |
| [e,01] iEXP/aRRkkk                                         |           |                               |
| logicb R9,R3,7,15,2                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<shiftl-t>> [[shiftl-d][_shiftl_]] Rd,Re,h                | eD02 E00H | Rd := Re shl h                |
| [e,02] iEXP/aRRk                                           |           |                               |
| shiftl R6,R7,2                                             | e602 7002 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<shiftr-t>> [[shiftr-d][_shiftr_]] Rd,Re,h                | eD03 E..H | Rd := Re shr h                |
| [e,03] iEXP/aRRk                                           |           |                               |
| shiftr R6,R7,2                                             | e603 7002 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<extract-t>> [[extract-d][_extract_]] Rd,Re,f,g,h         |           | Rd.f~g :=                     |
| [e,04] iEXP/aRRkkk                                         |           |                               |
| extract R6,R7,5,15,2                                       | eD04 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<save-t>> [[save-d][_save_]] Rd,Re,gh[Rf]                 |           | M[Rf+gh] := Rd,               |
| [e,05] iEXP/aRRX                                           |           | M[Rf+gh+1] := R{d+1}          |
|                                                            |           | M[Rf+gh+e-d] := Re            |
| save R3,R9,50[R13]                                         | eD05 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<restore-t>> [[restore-d][_restore_]] Rd,Re,gh[Rf]        |           | Rd := M[Rf+gh],               |
| [e,06] iEXP/aRRX                                           |           | ...,                          |
|                                                            |           | Re := M[Rf+gh+e-d]            |
| restore R3,R9,50[R13]                                      | eD06 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<push-t>> [[push-d][_push_]] Rd,Re,Rf                     |           | M[ea] := Rd, Rb++             |
| [e,07] iEXP/aRRR                                           |           |                               |
| push R4,R13,R8                                             | eD07 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<pop-t>> [[pop-d][_pop_]] Rd,Re,Rf                        |           | Rb--, Rd := M[ea]             |
| [e,08] /aRRR                                               |           |                               |
| pop R4,R13,R8                                              | eD08 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<top-t>> [[top-d][_top_]] Rd,Re,Rf                        |           | Rb--, Rd := M[ea]             |
| [e,09] iEXP/aRRR                                           |           |                               |
| top R4,R13,R8                                              | eD09 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<brc0-t>> [[brc0-d][_brc0_]] brc0 Rd,e,fgh                | eD0aA     | if R[d].e=0                   |
| [e,0a] iEXP/ ??                                            |           | then pc:=pc+fgh               |
| logicf R9,R3,7,15,2                                        | eD0a 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<brc1-t>> [[brc1-d][_brc1_]] Rd,e,fgh                     | eD0b      | if R[d].e=1                   |
| [e,0b] ???                                                 |           | then pc:=pc+fgh               |
| logicf R9,R3,7,15,2                                        | eD0b 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<brz-t>> [[brz-d][_brz_]] Rd,efgh                         | eD0c      | if R[d]=0000                  |
| [e,0c] ???                                                 |           | then pc:=pc+efgh              |
| logicf R9,R3,7,15,2                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<brnz-t>> [[brnz-d][_brnz_]] Rd,efgh                      | eD0d      | if R[d]!=0000                 |
| [e,0d] ???                                                 |           | then pc:=pc+efgh              |
| logicf R9,R3,7,15,2                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| <<dispatch-t>> [[dispatch-d][_dispatch_]] Rd,efgh          | eD0e      | pc :=                         |
| [e,0e] iEXP/?????                                          | efgh      | M[pc+min(Rd,efgh)]]           |
| dispatch bla                                               | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *PRIV* <<getctl-t>> [[getctl-d][_getctl_]] Rd,e            | eD0f      | Rd := Sc privileged           |
| [e,0f] iEXP/axxxxx                                         | e...      |                               |
| getctl bla                                                 | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *PRIV* <<putctl-t>> [[putctl-d][_putctl_]] Rd,e            | eD10      | Sc := Rd privileged           |
| [e,10] iEXP/axxxx                                          | e...      |                               |
| putctl bla                                                 | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *PRIV* <<resume-t>> [[resume-d][_resume_]]                 | eD11      | pc := ipc,                    |
| [e,11] iEXP/axxxx                                          |           | status := istatus             |
| resume bla                                                 | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *PRIV* <<timeron-t>> [[timeron-d][_timeron_]] Rd,efgh      | eD12      | timer := efgh,                |
| [e,12]                                                     | efgh      | start timer                   |
| timeron bla                                                | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *PRIV* <<timeroff-t>> [[timeroff-d][_timeroff_]] Rd,efgh   | eD13      | stop timer                    |
| [e,13]                                                     | efgh      |                               |
| timeroff bla                                               | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| [e,14]...[e,1f] reserved                                   |           |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xadd-t>> [[xadd-d][xadd]] Rd,Re,Rf                 | eD20      | Rd := Re+Rf                   |
| [e,20] iEXP/aRRR                                           |           |                               |
| xadd R8,R12,R5                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xsub-t>> [[xsub-d][xsub]] Rd,Re,Rf                 | eD21      | Rd := Re+rf                   |
| [e,21] iEXP/aRRR                                           |           |                               |
| xsub R8,R12,R5                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xmul-t>> [[xmul-d][xmul]] Rd,Ra,Rb                 | eD22      | Rd := Re+rf                   |
| [e,22] iEXP/aRRR                                           |           |                               |
| xmul R8,R12,R5                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xdiv-t>> [[xdiv-d][xdiv]] Rd,Ra,Rb                 | eD23      |                               |
| [e,23] iEXP/aRRR                                           |           |                               |
| xdiv R8,R12,R5                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xcmp-t>> [[xcmp-d][xcmp]] RD,RE                    | eD24 E000 |                               |
| [e,24] iEXP/aRR                                            |           |                               |
| xcmp R12,R5                                                | ec24 5000 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xaddc-t>> [[xaddc-d][xaddc]] Rd,Ra,Rb              | eD25      |                               |
| [e,25] iEXP/aRRR                                           |           |                               |
| xaddc R8,R12,R5                                            | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xmuln-t>> [[xmuln-d][xmuln]] Rd,Ra,Rb              | eD26      |                               |
| [e,26] iEXP/aRRR                                           |           |                               |
| xmuln R8,R12,R5                                            | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xdivn-t>> [[xdivn-d][xdivn]] Rd,Re,Rf              |           |                               |
| [e,27] iEXP/aRRR                                           |           |                               |
| xdivn R8,R12,R5                                            | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xjump-t>> [[xjump-d][xjump]] Rd,disp[Ra]           | FdaD      | if $R_d=0$ then pc := ea      |
| [e,28] iEXP/aRX                                            | disp      |                               |
| xjump R4,$0e3a[R14]                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xjal-t>> [[xjal-d][xjal]] Rd,disp[Ra]              | FdaF      | $R_d := pc, pc := ea$         |
| [e,29] iEXP/aRX                                            | disp      |                               |
| xjal R4,$0e3a[R14]                                         | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xjumpz-t>> [[xjumpz-d][xjumpz]] Rd,disp[Ra]        | FdaD      | if $R_d=0$ then pc := ea      |
| [e,2a] iEXP/aRX                                            | disp      |                               |
| xjumpz R4,$0e3a[R14]                                       | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xjumpnz-t>> [[xjumpnz-d][xjumpnz]] Rd,disp[Ra]     | FdaE      | if $R_d \neq 0$ then pc := ea |
| [e,2b] iEXP/aRX                                            | disp      |                               |
| xjumpnz R4,$0e3a[R14]                                      | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xsave-t>> [[xsave-d][xsave]] Rd,Re,gh[Rf]          |           | Rd.f~g :=                     |
| [e,2c] iEXP/aRRX                                           |           |                               |
| xsave R3,R9,50[R13]                                        | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xrestore-t>> [[xrestore-d][xrestore]] Rd,Re,gh[Rf] |           | Rd.f~g :=                     |
| [e,2d] iEXP/aRRX                                           |           |                               |
| xrestore R3,R9,50[R13]                                     | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xpush-t>> [[xpush-d][xpush]] Rd,Re,Rf              |           | Rd.f~g :=                     |
| [e,2e] iEXP/aRRR                                           |           |                               |
| xpush R4,R13,R8                                            | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xpop-t>> [[xpop-d][xpop]] Rd,Re,Rf                 |           | Rd.f~g :=                     |
| [e,2f] iEXP/aRRR                                           |           |                               |
| xpop R4,R13,R8                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
| *S32* <<xtop-t>> [[xtop-d][xtop]] Rd,Re,Rf                 |           | Rd.f~g :=                     |
| [e,30] iEXP/aRRR                                           |           |                               |
| xtop R4,R13,R8                                             | e900 37f2 |                               |
|------------------------------------------------------------+-----------+-------------------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
<<EXP3 instruction reference>>
** EXP3 instructions

#+BEGIN_EXPORT html
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: EXP3 instruction reference
#+ATTR_LATEX: :environment longtable :align l|l|l
| *Assembly*                                              | *Code*         | *Effect*         |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xjumpc0-t>> [[xjumpc0-d][xjumpc0]] m,disp[Ra]   | eD40 EFGH pqrs | if Rd==0         |
| [e,40] iEXP3/amX                                        |                | then pc := ea    |
| xjumpc0 25,$0ea3a[R14]                                  | eD00 37f2 pqrs |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xjumpc1-t>> [[xjumpc1-d][xjumpc1]] m,disp[Ra]   | Ed00 abcd aabc | if Rd==0         |
| [e,41] iEXP3/amX                                        |                | then pc := ea    |
| xjumpc1 25,$0ea3a[R14]                                  | e900 37f2      |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xlogicf-t>> [[xlogicf-d][xlogicf]] Rd,Rb,m,n,h  | FdbD abcd efgh | Rd.m:n :=        |
| [e,42] iEXP3/aRRmmk                                     |                | h(Rd.m:n,Rb.m:n) |
| xlogicf R9,R3,7,25,20                                   | e900 37f2      |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xlogicb-t>> [[xlogicb-d][xlogicb]] Rd,Rb,m,n,h  | FdbE efgh pqrd | Rd.f :=          |
| [e,43] iEXP3/aRRmmk                                     |                | h(Rd.f,Re.g)     |
| xlogicb R9,R3,7,25,20                                   | e900 37f2 abcd |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xshiftl-t>> [[xshiftl-d][xshiftl]] Rd,Re,m      | FdvF EFGH PQRS | Rd.f~g :=        |
| [e,44] iEXP3/aRRmmk                                     |                |                  |
| xshiftl R6,R7,17                                        | e900 37f2 abcd |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xshiftr-t>> [[xshiftr-d][xshiftr]] Rd,Re,m      | FdvF abcd abcd | Rd.f~g :=        |
| [e,45] iEXP3/aRRmmk                                     |                |                  |
| xshiftr R6,R7,27                                        | e900 37f2 abcd |                  |
|---------------------------------------------------------+----------------+------------------|
| *S32* <<xextract-t>> [[xextract-d][xextract]] Rd,Re,m,n | FdvF ABCD ABCD | Rd.f~g :=        |
| [e,46] iEXP3/aRRmmk                                     |                |                  |
| xextract R6,R7,18,20,16                                 | e900 37f2 abcd |                  |
|---------------------------------------------------------+----------------+------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
The 32 bit pseudoinstructions are similar to their 16 bit
counterparts.  Note that xorw is a 16-bit exclusive or, while
xxorw is a 32-bit exclusive or.

#+BEGIN_EXPORT html
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: Pseudoinstructions
|-----------------------------+---------------------+-----------+----------------------|
| Pseudo                      | Instruction         | Code      | Effect               |
|-----------------------------+---------------------+-----------+----------------------|
...
|-----------------------------+---------------------+-----------+----------------------|
| <<xinvw-t>> [[xinvw-d][xinvw]] Rd        | logicf Rd,Re,0,15,1 | Ed01,e0F1 | invert Rd            |
| <<xandw-t>> [[xandw-d][xandw]] Rd,Re     | logicf Rd,Re,0,15,1 | Ed01,e0F1 | Rd := Rd $\wedge$ Re |
| <<xorw-t>> [[xorw-d][xorw]] Rd,Re       | logicf Rd,Re,0,15,1 | Ed01,e0F1 | Rd := Rd $\vee$ Re   |
| <<xorw-t>> [[xorw-d][xxorw]] Rd,Re      | logicf Rd,Re,0,15,1 | Ed01,e0F1 | Rd := Rd xor Re      |
| <<xinvf-t>> [[xinvf-d][xinvf]]n Rd,f,g   | logicf Rd,Re,0,15,1 | Ed01,e0F1 | invert field         |
| <<xandf-t>> [[xandf-d][xandf]] Rd,Re,f,g | logicf Rd,Re,0,15,1 | Ed01,e0F1 | and field            |
| <<xorf-t>> [[xorf-d][xorf]] Rd,Re,f,g   | logicf Rd,Re,0,15,1 | Ed01,e0F1 | or field             |
| <<xxorf-t>> [[xxorf-d][xxorf]] Rd,Re,f,g | logicf Rd,Re,0,15,1 | Ed01,e0F1 | xor field            |
| <<xinvb-t>> [[xinvb-d][xinvb]] Rd,Re,f,g | logicb Rd,Re,f,g,1  | Ed01,e0F1 | invert bit           |
| <<xandb-t>> [[xandb-d][xandb]] Rd,Re,f,g | logicb Rd,Re,f,g,1  | Ed01,e0F1 | and bit              |
| <<xorb-t>> [[xorb-d][xorb]] Rd,Re,f,g   | logicb Rd,Re,f,g,1  | Ed01,e0F1 | or bit               |
| <<xxorb-t>> [[xxorb-d][xxorb]] Rd,Re,f,g | logicb Rd,Re,f,g,1  | Ed01,e0F1 | xor bit              |
|-----------------------------+---------------------+-----------+----------------------|
#+BEGIN_EXPORT html
</div>
#+END_EXPORT
...
<<xorb-d>>
<<xxorb-d>>
Descriptions for 32-bit pseudoinstructions
...
There are four machine language formats: RRR, RX, EXP, and EXP3.
However, there is a larger number of assembly language statement
formats.
- The =add= instruction is =RRR= format, and its assembly language
  format is =aRRR=.
- The =cmp= instruction is also =RRR= format, but the machine
  ignores the first operand (the D field).  That field is omitted
  in the assembly language statement, so to compare R2 and R3 you
  write =cmp R2,R3=.  This statement has assembly format =aRR= and
  it generates a =RRR= machine language instruction.
- Assembler directives are not instructions at all; they just give
  additional information to the assembler.  Some of these have
  assembler formats that don't correspond to any instruction.

#+BEGIN_EXPORT html
<div style="overflow-x: auto">
#+END_EXPORT
#+CAPTION: Assembly Language Statement Formats
| Asm Format | Example             |
|------------+---------------------|
| aRRR       | add R1,R2,R3        |
| aRR        | cmp R2,R3           |
...
|------------+---------------------|
| aRRk       | shiftl R2,R3,5      |
| aRRkk      | andf R3,R4,8,3      |
| aRRkkk     | logicf R2,R3,8,3,5  |
| aRRX       | save R2,R9,100[R14] |
| aRkK       | brc0                |
| aRK        | brz                 |
| aRk        | dispatch            |
| aRC        | getctl              |
...
- Comment.  All text that either follows white space after the
  operands field, or that follows a semicolon (;), is a comment,
  and is ignored by the assembler.  If one or more of the other
  fields (label, operation, operands) is missing, the comment must
  be preceded by a semicolon to prevent it from being interpreted
  as operands.  The rule is: all text after a semicolon is a
  comment, and all text after white space following operands is a
  comment.  A statement where the first non-space character is a
  semicolon is a full line comment.  If the statement has no
  operands, then all text after the operation field is a comment.
  It is good practice always to begin a comment with a semicolon.
...
x.asm.txt, where the basename x is the name of the module defined
in the file.  If the file contains a module statement =modname
module=, then the file's basename should be modname.  For example,
the file Heapsort.asm.txt might contain the statement =Heapsort
module=.  If there is no module statement, then the basename is
arbitrary.
...
An object file x.obj.txt, containing the object code for module x,
may have a corresponding metadata file x.md.txt.

# The format of the metadata is identical for object and
# executable; the reason for the distinction is that the user
# might have a program with main program foo.asm.txt, and later
# give the executable the same name foo.  In that case, there will
# be separate metadata files for the object and the executable.
...
* Projects with Sigma16

- Sigma16 is intended for use in teaching, self-learning, and
  research
- Projects involving Sigma16 are encouraged
- These may involve
  - Writing assembly language programs,using the existing sofware
  - Modifying the existing software, for example you might add some
    new instructions
  - Writing new software from scratch, for example writing a new
    emulator, or a complete new system
- In all cases, it's important to cite Sigma16 properly, and to
  adhere to the sofware license
- One important topic in software engineering, which sometimes
  doesn't get the attention it deserves in courses, is software
  licensing and copyright.
- Documentation
  - There needs to be some documentation.  At a minimum, this can
    be a short README file in your source directory.  Ideally
    there should be a user guide for your project.
  - In the documentation for your project, you need to cite
    Sigma16.
  - State clearly what you have used from Sigma16, and state what
    is your own code.  When you are finishing your project, you
    may realise that these points differ from what you intended at
    the outset.
- License
  - Sigma16 is currently licensed under the GPL3 license.
  - This is free software, but if you incorporate any of the code
    or text from Sigma16 in your project, you must abide by the
    requirements of the GPL3 license.  That means you must cite
    the original code, and you must release your software
    publically under the GPL3 license (so others can copy and
    modify your code too).
  - If your code contains anything from Sigma16, you cannot
    release your code without a license.
- Some tips for student projects
  - State clearly what the aim of your project is.  It's quite
    reasonable to implement a new version of something already
    provided by the Sigma16 software; for example you might want
    to write your own assembler.  It isn't necessary to pick on
    some small feature and focuse exclusively on that.
  - Toward the end of your project, revisit the goals.  You may
    find that the initial goals are no longer quite right.
  - If you say that Sigma16 doesn't provide feature x, and you do
    provide it, check that Sigma16 doesn't actually provide x, and
    also check that you do.
...
- *Permalink to the latest live release*
  - This permalink to the Sigma16 Home Page will always give you
    the latest release:\\
    [[https://jtod.github.io/home/Sigma16]]
  - The github page for Sigma16 contains the current development
    code, which changes frequently and is not stable.\\
    https://github.com/jtod/Sigma16
  - The github page also contains links to versions that were
    released since 2019.  Versions of Sigma16 from the first
    release in 2010 up to 2018 are not available online.
  - All releases prior to the latest live release are obsolete.
    They are not supported: always use the latest live release.
  
- v3.9.0, February 2025
  - Refinements to user guide and user interface
  - Further development of EXP instructions
  - Used at Glasgow in CS1 Spring 2025, CA Autumn 2025, CS1 Spring
    2026
...
- v3.2.1, February 2021
  ...
  - Instead, use the =cmp= instruction, which sets the condition
    code (=R15=), and then use a conditional jump instruction.
    The machine instructions are =jumpc0= and =jumpc1=.  It is
    easier to use a pseudoinstruction that uses the right
    condition code bit index; these are =jumplt=, =jumple=,
    =jumpeq=, =jumpne=, =jumpge=, =jumpgt=.
...
- v0.1.7, October 2011
  ...
  - However, this version is obsolete and it's recommended to use
    ...
    [[https://jtod.github.io/home/Sigma16/]].\\
    The current release has an improved architecture, is portable,
    has extensive documentation, is much faster, includes better
    software tools, and has a better user interface.
...
** Design Rationale for Sigma16

Goals
- Small but realistic instruction set
- Tiny core that supports ordinary programming
- Full architecture supports operating system kernel
- Digital circuit that fullly implements the core
- Understandable and readable machine language
- Modules, lining, relocation, booting
- Emphasise connections between levels of abstraction
- Software tools: assembler, linker, emulator, IDE
- Software tools should run in browser, without downloading or
  installing anything
- Clear relationship between assembly language and machine
  language (especially for the core)
- System should make crystal clear the distinction between
  assembly language and machine language
- Show clearly that the machine executes machine language, not
  assembly language
- User Guide always available in the browser app, and it is always
  visible when the app launches

Non-goals
- Full commercial architecture (too complicated to cover in depth
  in reasonable time for beginners)
- Large set of word sizes and primitive data types
- 
...
- verbatim =equals=
- bold *stars*
- code ~lea R3,$0b3a[R14]~
- math $\sqrt{\pi}$
- italic /slashes/
- underlined _underline_
- 2strike-through +strike through+
- C-c C-x C-f x put marker around region  =abcd=

** Links

See the [[mul-d][mul]] instruction.  This is an internal reference
to mul, and the target is defined by ~<<mul>>~, which is placed
immediately before the place you want the link to go to.

Also see [[mul-d]].  And there is a section on [[div-d][div]].
