...
on words.  These are "bitwise" operations: bit $i$ of the result
depends on bit $i$ of each operand, and does not depend on bits at any
other index.

These instructions take one or two operands, calculate the result, and
place the result in the first operand register.  There is not a
separate destination register, as in the add instruction; instead, the
first operand register is overwritten.

invw R3 inverts the bits in R3:
...
andw R1,R2 calculates the logical /and/ of R1 and R2, and places the
result in the destination register R1.
...
The invw, andw, orw, and xorw instructions are common special cases.
The architecture provides two general instructions, logicf and logicb,
...
pseudoinstructions: the assembler generates an equivalent logicf
instruction.  See the section below on Logic for a complete
description.
...
* Dummy section after ISA summary tables
